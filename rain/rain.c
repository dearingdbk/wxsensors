 ...
 * Date:     07/01/2026
 * Version:  1.0
 * Purpose:  Emulates a Campbell Scientific CS700H Heated Rain Gauge over SDI-12.
 *           This program sets up a serial connection with two threads:
 *            - Receiver thread: parses and responds to incoming SDI-12 commands
 *            - Sender thread: periodically transmits tip counts when polled
 *
 *           Supported commands (per SDI-12 protocol):
 *             aI!      - Send identification
 *             aM!      - Start measurement (returns 9 values in 0 seconds)
 *             aD0!     - Send data from last measurement
 *             aR0!     - Continuous measurement (same as aD0!)
 *             aC!      - Start concurrent measurement
 *             aAb!     - Change address from 'a' to 'b'
 *             ?!       - Address query
 *             aX20!    - Get/set external control (0=auto, 1=manual)
 *             aX21!    - Get/set heater on/off (manual mode only)
 *             aX22!    - Get/set snow sensor enable (0=disabled, 1=enabled)
 *             aX23!    - Get/set ActiveOn temperature (default +4.0°C)
 *             aX24!    - Get/set ActiveOff temperature (default +5.0°C)
 *             aX25!    - Get/set LowOff temperature (default -20.0°C)
 *             aX26!    - Get/set funnel SetPoint temperature (default +10.0°C)
 *             aX27!    - Get/set snow run-on time (default 18 mins)
 *             aX28!    - Get/set units (0=°C, 1=°F)
 *             aX29!    - Get/set cycle enable (manual mode only)
 *
 *           Output format for aI! command:
 *             a12HydrServTBHeat4.1XXXXX<CR><LF>
 *           Where:
 *             a     = SDI-12 address
 *             12    = SDI-12 version 1.2
 *             HydrServ = Manufacturer ID
 *             TBHeat = Model name
 *             4.1   = Software revision
 *             XXXXX = 5-digit sensor ID
 *
 *           Output format for aD0! command:
 *             a+AA.A+BB.B+U+S+E+H+C+Y+TT<CR><LF>
 *           Where:
 *             AA.A = Ambient temperature (°C or °F)
 *             BB.B = Block temperature (°C or °F)
 *             U    = Units (0=°C, 1=°F)
 *             S    = Snow detected (0=no, 1=yes)
 *             E    = Snow sensor enabled (0=no, 1=yes)
 *             H    = Heater on (0=off, 1=on)
 *             C    = Control mode (0=auto, 1=manual)
 *             Y    = Cycle enabled (0=no, 1=yes)
 *             TT   = Heater cycle time left (minutes)
 *
 *           Pulse output: Reed switch closure per 0.01" (0.254mm) tip
 *
 * Usage:    rain <data_file> [serial_port] [baud_rate] [RS422|RS485]
 ...
 *           - Technology: Heated tipping bucket with internal siphon
 *           - Orifice diameter: 200mm (7.87 inches)
 *           - Resolution: 0.01 inches (0.254mm) per tip
 *           - Measurement range: 0-700 mm/hr (0-27.6 in/hr)
 *           - Accuracy: ±2% @ <250 mm/hr, ±3% @ 250-500 mm/hr
 *           - Operating temperature: -40°C to +70°C (heated)
 *           - Snow sensor active range: -20°C to +4°C
 *           - Funnel set point temperature: +10°C when heating
 *           - Communication: SDI-12, 1200 baud, 7 bits, even parity
 *           - Default SDI-12 address: 0
 *           - Pulse output: Dual reed switch closure
 *           - Heater power: 70W @ 12V (5.8A) when active
 *           - Snow run-on time: 18 minutes (default)
